...
<c:set var="measures" value="<%= MeasureManager.getAllBaseMeasuresByEntity(Measure.FACTORY_ENTITY) %>"></c:set>
<c:choose>
	<c:when test="${fn:length(measures) gt 0}">
	<ul>
	    <s:set name="factory" value="factory" scope="request"/>
        <c:forEach var="measure" items="${measures}">
            <c:set var="measureName" value="factory.measures.${measure.name}"></c:set>
            <c:if test="${measure.annotated == true}">
                <c:set var="measureName" value="factory.${measure.name}"></c:set>
            </c:if>
		    <li>
		        ...
		        <s:fielderror><s:param>error.factory.${measureName}</s:param></s:fielderror>
		    </li>
		</c:forEach>
	</ul>
	</c:when>
    <c:otherwise>
        <p><s:text name="label.factory.base_measures.lt0"></s:text></p>
    </c:otherwise>
</c:choose>
