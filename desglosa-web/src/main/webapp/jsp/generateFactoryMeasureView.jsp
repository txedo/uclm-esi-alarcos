...
<c:set var="measures" value="<%= MeasureManager.getAllMeasuresByEntity(Measure.FACTORY_ENTITY) %>"></c:set>
...
<c:choose>
	    ...
	    <ul>
	       <s:set name="factory" value="factory" scope="request"/>
            <c:forEach var="measure" items="${measures}">
                <c:set var="measureValue" value="factory.measures.${measure.name}"></c:set>
                <c:if test="${measure.annotated == true}">
                    <c:set var="measureValue" value="factory.${measure.name}"></c:set>
                </c:if>
	            <li>
	                <label class="key">
	                	${measure.name}
    ...
    <c:otherwise>
        <p><s:text name="label.factory.measures.lt0"></s:text></p>
    </c:otherwise>
</c:choose>
