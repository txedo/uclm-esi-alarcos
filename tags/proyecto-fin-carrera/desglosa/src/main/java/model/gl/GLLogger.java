...
public class GLLogger {
    ...
    public GLLogger() throws GLSingletonNotInitializedException {
        ...
        GLSingleton.getGL().glGetFloatv(GL2.GL_ALIASED_POINT_SIZE_RANGE,
                aliased_point_size_range, 0);
        GLSingleton.getGL().glGetFloatv(GL2.GL_SMOOTH_POINT_SIZE_RANGE,
                smooth_point_size_range, 0);
        GLSingleton.getGL().glGetFloatv(GL2.GL_SMOOTH_POINT_SIZE_GRANULARITY,
                smooth_point_size_granularity, 0);

        GLSingleton.getGL().glGetIntegerv(GL2.GL_MAX_ELEMENTS_VERTICES,
                elements_vertices, 0);
        GLSingleton.getGL().glGetIntegerv(GL2.GL_MAX_ELEMENTS_INDICES,
                elements_indices, 0);

        GLSingleton.getGL().glGetIntegerv(GL2.GL_STENCIL_BITS,
                stencilbuffer_bits, 0);

        GLSingleton.getGL().glGetIntegerv(GL2.GL_SAMPLE_BUFFERS,
                sample_buffers, 0);
    ...
    public void printToGL(int h, int w, float d)
        ...
        for (String line : lines) {
            GLUtils.renderBitmapString(0.0f, (vgap * lines.length)
                    - (vgap * counter), 1, line);
            counter++;
        }
        GLUtils.endOrtho();
