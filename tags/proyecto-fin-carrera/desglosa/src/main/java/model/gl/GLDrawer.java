...
public class GLDrawer implements GLEventListener, IGLConstants {
     ...
     * following sections: * A switch case in the display() function. * Instance
     * initialization at the init() function. * updateProjection() function. *
     * setSelectionMode() function, in case that the view implements a selection
     * mechanism. * getGLViewManager() function.
     */
    ...
    public void reshape(GLAutoDrawable glDrawable, int x, int y, int width, int height) {
        try {
            ...
            GLSingleton.getGL().glViewport(0, 0, this.screenWidth,
                    this.screenHeight);
            this.updateProjection();
        } catch (GLSingletonNotInitializedException e) {
            GLSingleton.init(glDrawable);
