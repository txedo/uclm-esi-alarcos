...
public class MyMouseAdapter extends MouseAdapter {
                    ...
                    new ActionListener() {
                        public void actionPerformed(ActionEvent evt) {
                            if (wasDoubleClick)
                                ...
                                drawer.setSelectionMode(true, e.getButton(), 1);
                        }
                    });
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (this.drawer.getCamera().isMousing()) {
            if (drawer.getViewManager(drawer.getViewLevel())
                    .isThreeDimensional()) {
                ...
                // } else {
                // throw new
                // RuntimeException("Event source neither Window nor Component: "+source);
                // }

                float nXDiff = this.currentMousePosition.getX()
                        - this.lastMousePosition.getX();
                float nYDiff = this.currentMousePosition.getY()
                ...
                case 1:
                    if (nYDiff < 0)
                        this.drawer.getCamera().moveForward((int) -nYDiff);
                    else if (nYDiff > 0)
                        this.drawer.getCamera().moveBackward((int) nYDiff);

                    if (nXDiff < 0)
                        this.drawer.getCamera().strafeLeft((int) -nXDiff);
                    else if (nXDiff > 0)
                        this.drawer.getCamera().strafeRight((int) nXDiff);
                    break;
                case 3:
                    if (nYDiff < 0)
                        this.drawer.getCamera().lookUp((int) -nYDiff);
                    else if (nYDiff > 0)
                        this.drawer.getCamera().lookDown((int) nYDiff);

                    if (nXDiff < 0)
                        this.drawer.getCamera().rotateLeft((int) -nXDiff);
                    else if (nXDiff > 0)
                        this.drawer.getCamera().rotateRight((int) nXDiff);
                    break;
                default:
                    break;
                }
