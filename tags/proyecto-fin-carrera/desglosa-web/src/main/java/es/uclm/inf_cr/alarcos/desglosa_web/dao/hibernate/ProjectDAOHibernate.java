...
public class ProjectDAOHibernate extends GenericDAOHibernate<Project, Long>
    ...
    public Project getProject(String name) throws ProjectNotFoundException {
        List projects = getHibernateTemplate().find(
                "from Project where name=?", name);
        if (projects == null || projects.isEmpty()) {
            throw new ProjectNotFoundException("project '" + name
                    + "' not found...");
        } else {
    ...
    public List<Project> getProjects() {
        return getHibernateTemplate().find(
                "from Project p order by upper(p.name)");
    }
