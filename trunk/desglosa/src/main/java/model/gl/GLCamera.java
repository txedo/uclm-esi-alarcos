...
public class GLCamera {
    ...
    public GLCamera(float xpos, float ypos, float zpos, float xviewdir,
            float yviewdir, float zviewdir) {
        this.mousing = false;
    ...
    public void render() throws GLSingletonNotInitializedException {
        ...
        GLSingleton.getGLU().gluLookAt(position.getX(), position.getY(),
                position.getZ(), viewPoint.getX(), viewPoint.getY(),
                viewPoint.getZ(), upVector.getX(), upVector.getY(),
                upVector.getZ());
    }
    ...
    // TODO para los zooms, mantener una maquina de estados que indica si cada
    // una
    ...
    private void rotateX(float angle) {
        ...
        // No recalculamos el upVector porque no queremos una camara totalmente
        // libre
    }
    ...
    private void rotateY(float angle) {
        ...
        Vector3f comp1 = viewDir.mult((float) Math.cos(angle * Math.PI / 180));
        Vector3f comp2 = rightVector.mult((float) Math.sin(angle * Math.PI
                / 180));
        viewDir = comp1.add(comp2).getNormalizedVector();

        // Recalculamos rightVector con el producto vectorial de viewDir y
        // upVector
        rightVector = viewDir.cross(upVector);
        // Recalculamos frontVector con el producto vectorial de upVector y el
        // nuevo rightVector
        frontVector = rightVector.cross(upVector).mult(-1);
    }
    ...
    public void lookBackward() {
        ...
        viewDir = new Vector3f(viewDir.getX() * -1, viewDir.getY(),
                viewDir.getZ() * -1);
    }
