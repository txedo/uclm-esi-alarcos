...
public class SubprojectDAOHibernate extends GenericDAOHibernate<Subproject, Long> implements SubprojectDAO {
	...
	public Subproject getSubproject(int id) throws SubprojectNotFoundException {
		Subproject sp = (Subproject) getHibernateTemplate().get(Subproject.class, id);
		if (sp == null) throw new SubprojectNotFoundException("subproject '" + id + "' not found...");
		return sp;
	}
	
	@SuppressWarnings("rawtypes")
	public Subproject getSubproject(String name) throws SubprojectNotFoundException {
		List subprojects = getHibernateTemplate().find("from Subproject where name=?", name);
	    if (subprojects == null || subprojects.isEmpty()) {
	    	throw new SubprojectNotFoundException("subproject '" + name + "' not found...");
	    } else {
	...
	public List<Subproject> getSubprojects() {
		return getHibernateTemplate().find("from Subproject sp order by upper(sp.name)");
	}
