...
tabla de un escenario

\imagenBorde{Cap5//it7-profile-communication-class-diagram}{0.7}{Diagrama de comunicación la configuración de un perfil de visualización}{fig:it7-profile-communication-class-diagram}
...
\lstinputlisting[language=XML,caption={Fragmento de un perfil de visualización en formato XML},label=lst:profile-snippet]{snippets//profile-snippet.xml}
...
Para abordar cuestiones más específicas, se modela el diagrama de clases que da forma a la estructura de los perfiles de visualización (Fig. \ref{fig:it7-profile-class-diagram}). La estructura diseñada se corresponde exactamente con la que se propone en el \textit{snippet} de la disciplina de análisis de esta misma iteración (Listado \ref{lst:profile-snippet}). Cabe destacar que, tal y como se detalla en dicho diagrama, todas las clases deberán ser debidamente anotadas con anotaciones de JAXB para que sea posible su \textit{serialización} a formato XML y su posterior escritura en un fichero.
