...
\begin{itemize}
	...
	\item Implementación de un foco de luz que de más realismo a la escena mostrada.
\end{itemize}
...
\imagenBorde{Cap5//it2-camera-usecase}{1.1}{Diagrama de casos de uso para la manipulación de la cámara}{fig:it2-camera-usecase}
...
\begin{tabular}{| >{\arraybackslash}m{15cm} |}
	...
	\hline 
\end{tabular}
\caption{Descripción del caso de uso \textit{Desplazar}}
...
\imagenBorde{Cap5//it2-camera-spotlight-class-diagram}{1.15}{Diagrama de clases de diseño para los CdU1.1 y CdU1.2}{fig:it2-camera-spotlight-class-diagram}

En cuanto al diseño de CdU1.5 se propone una arquitectura de vistas basada en herencia y polimorfismo. Así pues, se define una clase abstracta de tipo \textit{GLViewManager} (Gestor de Vistas GL) que implementará un conjunto de operaciones comunes a todas las vistas (por ejemplo, dibujar el suelo de la escena) y definirá una plantilla con métodos que deberán ser implementados por cada vista especializada, es decir, las clases que la extiendan. Además, por medio del patrón de diseño \textbf{Factory Method} -formado por la clase IViewManagerFactoryImpl y la interfaz IViewManagerFactory- se abstrae a la clase \textit{GLDrawer} de la creación específica de cada vista (Fig. \ref{fig:it2-scene-class-diagram}).

\imagenBorde{Cap5//it2-scene-class-diagram}{0.8}{Diagrama de clases de diseño para los CdU1.5}{fig:it2-scene-class-diagram}
...
En la figura \ref{} se muestra un breve diagrama de clases en la que se define la estructura de la anotación y su correspondiente analizador.
...
En este documento sólo se incluyen dos aspectos relativos a la implementación la cámara que resultan de vital importancia y que se corresponden con las funciones \textit{render()} y \textit{calibrate()} de la clase \textbf{GLCamera} (Listado \ref{lst:camera-render-calibrate}).
\begin{itemize}
	\item \textit{render()}. Esta función, que será ejecutada cada vez que se produzca cualquier tipo de alteración en la posición o dirección de enfoque de la cámara, se encarga de hacer efectivos los nuevos cálculos.
