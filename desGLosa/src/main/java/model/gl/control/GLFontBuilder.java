...
public class GLFontBuilder {
	...
	protected GLFontBuilder() throws GLSingletonNotInitializedException, IOException {
		textureLoader = new TextureLoader(new String[]{"src/main/resources/gl/Font2.bmp"});
	 ...
	 * @throws GLSingletonNotInitializedException 
	 */
	static public GLFontBuilder getInstance() throws GLSingletonNotInitializedException, IOException {
		if (null == _instance) {
		...
		for (int loop=0; loop<FONTS; loop++)					// Loop Through All 95 Lists
		{
			...
			GLSingleton.getGL().glNewList(base+loop, GL.GL_COMPILE);
				GLSingleton.getGL().glBegin(GL.GL_QUADS);
					...
					GLSingleton.getGL().glVertex2f(LETTER_GAP,0);    // Vertex Coord (Bottom Right)
					GLSingleton.getGL().glTexCoord2f(cx+0.0625f,1-cy);   // Texture Coord (Top Right)
					GLSingleton.getGL().glVertex2f(LETTER_GAP, LETTER_GAP);	// Vertex Coord (Top Right)
					GLSingleton.getGL().glTexCoord2f(cx,1-cy);   // Texture Coord (Top Left)
					GLSingleton.getGL().glVertex2f(0, LETTER_GAP);    // Vertex Coord (Top Left)
	...
	{
		GLSingleton.getGL().glDeleteLists(base,FONTS);						// Delete All 256 Display Lists
	}

	
	public void glPrint(float x, float y, String text, int set, boolean centered) throws GLSingletonNotInitializedException			// Where The Printing Happens
	{	
        if (set > 1) set = 1;
