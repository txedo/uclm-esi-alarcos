...
public class Neighborhood extends City {
    ...
    protected Vector2f doLayout(Vector2f point) {
        // Once we know how many cols and rows it will have, we can calculate a
        // position for each flat
        float x = point.getX();
        float y = point.getY();
        GLObject obj = null;
        boolean done = false;
        ...
        this.width = this.cols * obj.getMaxWidth() + (this.cols - 1)
                * this.X_GAP;
        this.depth = this.rows * obj.getMaxDepth() + (this.rows - 1)
                * this.Y_GAP;
