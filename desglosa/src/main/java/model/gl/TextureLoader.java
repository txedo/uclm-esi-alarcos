...
public class TextureLoader {
    ...
    private int[] genTextures(int amount)
    ...
    private void makeRGBTexture(GLTexture img, int target, boolean mipmapped)
            throws GLSingletonNotInitializedException {
        if (mipmapped) {
            GLSingleton.getGLU().gluBuild2DMipmaps(target, GL2.GL_RGBA,
                    img.getWidth(), img.getHeight(), GL2.GL_RGBA,
                    GL2.GL_UNSIGNED_BYTE, img.getPixels());
        } else {
            GLSingleton.getGL().glTexImage2D(target, 0, GL2.GL_RGB8,
                    img.getWidth(), img.getHeight(), 0, GL2.GL_RGB,
                    GL2.GL_UNSIGNED_BYTE, img.getPixels());
        }
    }

    public void loadTexures(boolean useAlphaChannel, boolean useMipmaps,
            boolean linearFiltering, boolean isSkyBox)
        ...
        if (textureNames.length > 0) {
            ...
            for (int i = 0; i < textureNames.length; i++) {
                ...
                if (isSkyBox) {
                    GLSingleton.getGL().glTexParameteri(GL2.GL_TEXTURE_2D,
                    ...
                    if (useMipmaps) {
                        ...
                        if (linearFiltering) {
                            GLSingleton.getGL().glTexParameteri(
                                    GL2.GL_TEXTURE_2D,
