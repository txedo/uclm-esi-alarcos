...
public class GLSpotlight {
    ...
    public GLSpotlight(float r, float g, float b)
        ...
        if (GLSingleton.getGL() != null) {
            GLSingleton.getGL().glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT,
                    mat_ambient, 0);
            GLSingleton.getGL().glMaterialfv(GL2.GL_FRONT, GL2.GL_SPECULAR,
                    mat_specular, 0);
            GLSingleton.getGL().glMaterialfv(GL2.GL_FRONT, GL2.GL_DIFFUSE,
                    mat_diffuse, 0);
            GLSingleton.getGL().glMaterialf(GL2.GL_FRONT, GL2.GL_SHININESS,
                    mat_shininess);
            // GLSingleton.getGL().glColorMaterial(GL2.GL_FRONT,
            // GL2.GL_EMISSION);

            // Setup The Light
            GLSingleton.getGL().glLightfv(this.lightSource, GL2.GL_AMBIENT,
                    light_ambient, 0);
            GLSingleton.getGL().glLightfv(this.lightSource, GL2.GL_SPECULAR,
                    light_specular, 0);
            GLSingleton.getGL().glLightfv(this.lightSource, GL2.GL_DIFFUSE,
                    light_diffuse, 0);
            // Position and Render The Spotlight
            Vector3f pos = new Vector3f(position[0], position[1], position[2]);
            Vector3f dir = new Vector3f(direction[0], direction[1],
                    direction[2]);
            this.render(pos, dir);

            // Angle of the cone light emitted by the spot : value between 0 to
            // 180
            GLSingleton.getGL().glLightf(this.lightSource, GL2.GL_SPOT_CUTOFF,
                    105.0f);
            GLSingleton.getGL().glLightf(this.lightSource,
                    GL2.GL_SPOT_EXPONENT, 15.0f);

            // Light attenuation (default values used here : no attenuation with
            // the distance)
            GLSingleton.getGL().glLightf(this.lightSource,
                    GL2.GL_CONSTANT_ATTENUATION, 0.8f);
            GLSingleton.getGL().glLightf(this.lightSource,
                    GL2.GL_LINEAR_ATTENUATION, 0.005f);
            GLSingleton.getGL().glLightf(this.lightSource,
                    GL2.GL_QUADRATIC_ATTENUATION, 0.005f);

            GLSingleton.getGL().glLightModeli(GL2.GL_LIGHT_MODEL_LOCAL_VIEWER,
                    GL2.GL_TRUE);
            this.switchOn();
        } else
    ...
    public void render(Vector3f position, Vector3f viewDir)
        ...
        if (GLSingleton.getGL() != null) {
            ...
            // Position The Light
            GLSingleton.getGL().glTranslatef(position.getX(), position.getY(),
                    position.getZ());
            GLSingleton.getGL().glLightfv(this.lightSource, GL2.GL_POSITION,
                    this.position, 0); // Position The Light
            // Setup the spot direction
            this.direction = viewDir.toArray();
            GLSingleton.getGL().glLightfv(this.lightSource,
                    GL2.GL_SPOT_DIRECTION, this.direction, 0);
