...
\begin{tabular}{| p{5cm} | p{3.5cm} | p{3.5cm} |}
	\hline
		...
		Inshoring \\
	\cline{2-3}
		\raisebox{3,0 ex}[0ex][0ex]{\cellcolor[gray]{0.7}Externalización de servicios} &
		\cellcolor[gray]{0.9}Distintos países &
		Outsourcing \\
	\hline
		\cellcolor[gray]{0.7} &
		\cellcolor[gray]{0.9}Países vecinos &
		Nearshoring \\
	\cline{2-3}
		\raisebox{3,0 ex}[0ex][0ex]{\cellcolor[gray]{0.7}Traslación de fábricas} &
		\cellcolor[gray]{0.9}Países distantes &
		Offshoring \\
	\hline
\end{tabular}
...
Como se ha explicado en la sección \ref{sec:calidad}, realizar mediciones sobre el software es un desafío. Cuando se completa un proceso de medición, es conveniente generar un informe que muestre los resultados que se consideren de interés. Para que el estudio de estos resultados sea una tarea fácil, rápida y eficiente, se propone presentar dichos resultados con una representación visual. Pero toda la información obtenida durante el proceso de medición no es igualmente representativa para todos los usuarios, por tanto, es necesario filtrarla o permitir que diferentes tipos de audiencia puedan comprender información compleja.

En la literatura se encuentran distintas técnicas que permiten representar visualmente determinados tipos de información. \cite{fj98} y \cite{hvvw05} utilizan modelos en tres dimensiones para mostrar información estadística. \cite{wl07} emplea la metáfora de una ciudad en tres dimensiones para representar medidas software (Fig. \ref{fig:city-metaphore}). \cite{lanza03} combina vistas polimétricas con medidas software de las entidades conceptuales que intervienen en programas software orientado a objetos (clases, atributos, métodos...). Pero hay que controlar determinadas características para evitar introducir altos niveles de ruido en la visualización (Fig. \ref{fig:edge-noise}).

\imagenBorde{Cap3//city-metaphore}{0.55}{Metáfora de una ciudad para representar sistemas software}{fig:city-metaphore}

Antes de diseñar una representación visual, es necesario estudiar si un modelo 2D satisface las necesidades de la abstracción del mundo que se quiere escenificar, o si es necesario una escena más compleja en 3D. Por un lado, las escenas 2D son sencillas de implementar pero hacen que el usuario tenga la sensación de ver el sistema desde fuera y no se sienta inmerso en el mismo. Por otro lado, las escenas 3D sí producen esta sensación de inmersión pero pueden provocar que el usuario se desoriente. Para evitar este problema, se propone realizar la visualización 3D en base a la metáfora de una ciudad que el usuario puede explorar de forma interactiva \cite{wl07}.

No obstante, para llegar a la idea de la metáfora de una ciudad en 3D, los sistemas de visualización han pasado por un proceso de evolución que remonta a los gráficos en 2D. A lo largo de los años se han propuesto distintos diagramas y modelos que, según sus características, satisfacen mejor un tipo de datos u otro. A continuación, se muestra una síntesis de algunos de los modelos analizados en \cite{ducasse10}:

\begin{itemize}
	\item \textbf{Distribution Map}
	\item \textbf{Tree Map}
	\item \textbf{Tree Ring}
	\item \textbf{ICicle Plot}
	\item \textbf{File Dot}
	\item \textbf{Diagrama de Kiviat}
	\item \textbf{Dot Plot y Matrices de Correlación}
	\item \textbf{Matrices de Evolución}
	\item \textbf{Verso}
\end{itemize}
...
\imagen{Cap3//edge-noise}{0.55}{Ejemplo del nivel de ruido introducido por las aristas de un grafo}{fig:edge-noise}
